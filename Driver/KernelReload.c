	...
	__out PDWORD SystemKernelModuleSize)
{
	...
	__try
	{
		...
		*SystemKernelModulePath = ExAllocatePool(NonPagedPool, 260 * 2);
		...
		memcpy(*SystemKernelModulePath + wcslen(L"\\SystemRoot\\system32\\"),
				UnicodeKernelModule.Buffer,
				UnicodeKernelModule.Length);

		*SystemKernelModuleBase = (DWORD)pModuleList->smi[0].Base;	// 基地址
		*SystemKernelModuleSize = (DWORD)pModuleList->smi[0].Size;	// 大小
		ExFreePool(pModuleList);
...
//////////////////////////////////////////////////////////////////////////
BOOL GetDeviceObjectFromFileFullName(
	WCHAR *FileFullName,				// \SystemRoot\system32\ntkrnlpa.exe
	PDEVICE_OBJECT *RealDevice, 
	PDEVICE_OBJECT *DeviceObject)
{
...
NTSTATUS  KernelOpenFile(
	wchar_t *FileFullName,		// ntkrnlpa.exe 的路径	(\SystemRoot\system32\ntkrnlpa.exe)
	PHANDLE FileHandle,			// 返回的句柄
	...
	BYTE **ImageModuleBase)
{
	...
	SizeOfImage			= ImageNtHeaders->OptionalHeader.SizeOfImage;		// 映像装入内存后的大小
	SizeOfHeaders		= ImageNtHeaders->OptionalHeader.SizeOfHeaders;		// ms-dos + ntheader + section header
	...
	// 拷贝 ms-dos, nt header, section header 数据
...
PVOID MiFindExportedRoutine (
	IN PVOID DllBase,			// ntkrnlpa.exe pE 数据的基地址
	BOOL ByName,				// TRUE
	IN char *RoutineName,		// 函数名称
	DWORD Ordinal)				// 函数索引
{
	...
	NameTableBase = (PULONG)((PCHAR)DllBase + (ULONG)ExportDirectory->AddressOfNames/* 函数名称 RVA */);

	// Initialize the pointer to the array of USHORT ordinal numbers.
	NameOrdinalTableBase = (PUSHORT)((PCHAR)DllBase + (ULONG)ExportDirectory->AddressOfNameOrdinals/* 函数入口地址的索引 RVA */);

	AddressTableBase = (PULONG)((PCHAR)DllBase + (ULONG)ExportDirectory->AddressOfFunctions/* 函数入口地址 RVA */);
	...
	Low = 0;
	Middle = 0;
	High = ExportDirectory->NumberOfNames/* 以名称导出的函数总数 */ - 1;
...
///////////////////////////////////////////////////////////////////////////////
BOOL FixImportTable(
	BYTE *ImageBase,				// ntkrnlpa.exe 文件的数据
	DWORD ExistImageBase,			// ntkrnlpa.exe 的基地址
	...
	while (ImageImportDescriptor->OriginalFirstThunk && ImageImportDescriptor->Name)
	{
		...
		while (ImageThunkData->u1.Ordinal)
		{
			...
			if (IMAGE_SNAP_BY_ORDINAL32(ImageThunkData->u1.Ordinal))
			{
				...
				FirstThunk->u1.Function = FunctionAddress;	// PE 加载后指向的实际的地址
			}
			else	//函数名导入
			{
...
///////////////////////////////////////////////////////////////////////////////
BOOL PeLoad(
	WCHAR *FileFullPath,			// ntkrnlpa.exe 的路径	(\SystemRoot\system32\ntkrnlpa.exe)
	BYTE **ImageModeleBase,			// 返回 PE数据的地址
	PDRIVER_OBJECT DeviceObject,	// 驱动对象
	DWORD ExistImageBase)			// ntkrnlpa.exe 的基地址
{
	NTSTATUS			Status;
	HANDLE				hFile;
	...
	// 获取 ntkrnlpa.exe 中的节的数据，通过 ImageBase 返回
	...
	// 修改 ntkrnlpa.exe 导入所有模块加载后的地址，存入 ImageBase 指向的导入表中
