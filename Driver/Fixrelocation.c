...
///////////////////////////////////////////////////////////////////////////////
PVOID RtlpImageDirectoryEntryToData32 (
	IN PVOID Base,							// PE buffer
	IN BOOLEAN MappedAsImage,				// TRUE
	IN USHORT DirectoryEntry,				// IMAGE_DIRECTORY_ENTRY_IMPORT
	OUT PULONG Size,						// return
	PIMAGE_NT_HEADERS32 NtHeaders)			// Nt header
{
	ULONG	DirectoryAddress;

	// 校验数据目录的索引
	if (DirectoryEntry >= NtHeaders->OptionalHeader.NumberOfRvaAndSizes /* 数据目录表的个数 */ ) 
	{
		return( NULL );
	}
...
///////////////////////////////////////////////////////////////////////////////
PVOID RtlpImageDirectoryEntryToData64 (
	IN PVOID Base,
	...
	OUT PULONG Size)			// return
{
	...
	{
		return (RtlpImageDirectoryEntryToData32(
					Base,
					MappedAsImage,
					DirectoryEntry,
					Size,
	...
	{
		return (RtlpImageDirectoryEntryToData64(
					Base,
					MappedAsImage,
					DirectoryEntry,
					Size,
...
PIMAGE_BASE_RELOCATION LdrProcessRelocationBlockLongLong(
	IN ULONG_PTR VA,		// 重定位页
	IN ULONG SizeOfBlock,	// 重定位数组的个数
	IN PUSHORT NextOffset,	// 重定位数组的起始地址
	IN LONGLONG Diff)		// 地址差
{
	PUCHAR		FixupVA;
	USHORT		Offset;
	LONG		Temp;
...

{
    ...
    NTSTATUS			Status = 0;
    const ULONG			ValidFlags = RTL_IMAGE_NT_HEADER_EX_FLAG_NO_RANGE_CHECK;
...
BOOL FixBaseRelocTable (
	PVOID NewImageBase,		// ntkrnlpa.exe 的 PE 数据
	DWORD ExistImageBase)	// ntkrnlpa.exe 的基地址
{
	...
	{   
        case IMAGE_NT_OPTIONAL_HDR32_MAGIC:		// 32位 PE
            OriginalImageBase = ((PIMAGE_NT_HEADERS32)NtHeaders)->OptionalHeader.ImageBase;
            break;
        case IMAGE_NT_OPTIONAL_HDR64_MAGIC:		// 64位 PE
    ...
    NextBlock = (PIMAGE_BASE_RELOCATION)A_Protect_RtlImageDirectoryEntryToData(NewImageBase, TRUE, IMAGE_DIRECTORY_ENTRY_BASERELOC, &TotalCountBytes);
    ...
    while (TotalCountBytes)
	{
        ...
        if (!(NextBlock = LdrProcessRelocationBlockLongLong(VA/*重定位页*/, SizeOfBlock/* 重定位数组的个数 */, NextOffset/* 重定位数组的起始地址 */, Diff/* 地址差 */)))
		{
