...
///////////////////////////////////////////////////////////////////////////////
BOOL GetOriginalKiServiceTable(
	BYTE *NewImageBase,				// ntkrnlpa.exe PE 数据
	DWORD ExistImageBase,			// ntkrnlpa.exe 的基地址
	DWORD *NewKiServiceTable)		// 申请的 SSDT 函数的大小
{
...
///////////////////////////////////////////////////////////////////////////////
BOOL InitSafeOperationModule(
	PDRIVER_OBJECT pDriverObject,	// 驱动对象
	WCHAR *SystemModulePath,		// ntkrnlpa.exe 的路径	(\SystemRoot\system32\ntkrnlpa.exe)
	ULONG KernelModuleBase)			// ntkrnlpa.exe 的基地址
{
	...
	//
	OriginalKiServiceTable = ExAllocatePool(NonPagedPool, KeServiceDescriptorTable->TableSize/* 函数个数 */ * sizeof(DWORD));
...
NTSTATUS ReLoadNtos(PDRIVER_OBJECT DriverObject, DWORD RetAddress)
{
	...
	// 获取 ntkrnlpa.exe 模块的路径，基地址，大小
	if (!GetSystemKernelModuleInfo(
			&SystemKernelFilePath,		// ntkrnlpa.exe 的路径 (\SystemRoot\system32\ntkrnlpa.exe)
			&SystemKernelModuleBase,	// ntkrnlpa.exe 的基地址
			&SystemKernelModuleSize))	// ntkrnlpa.exe 的大小
	{
	...
	if (InitSafeOperationModule(
			DriverObject,				// 驱动对象
			SystemKernelFilePath,		// ntkrnlpa.exe 的路径
			SystemKernelModuleBase))	// ntkrnlpa.exe 的基地址
	{
		...
		if (!RMmIsAddressValid || !RPsGetCurrentProcess)
		{
